...
class ProviderDAOImpl(is: InputStream)
  ...
  protected def load(is: InputStream) {
    ...
    val sheet = workbook.getSheet(SheetNane)
    
    if (sheet == null) {
      logger.error("Sheet " + SheetNane + " does not " +
        "exist in the file specified")
      throw new IllegalArgumentException(s"Sheet '${SheetNane}' does not " +
        "exist in the file specified")
    }
